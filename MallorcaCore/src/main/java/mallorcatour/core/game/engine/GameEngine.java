...
public class GameEngine implements GameContext {
	...
	public GameEngine(Player player1, Player player2, IGameObserver observer, HandEvaluator equilator, String debug) {
	...
	private HandSummary deal(int handNumber, boolean changeBlind) {
		...
		nonUsedCards = new ArrayList<>(Deck.getCards());
	...
	protected void dealCards(Player player, OpenPlayerInfo playerInfo) {
	...
	Player otherThan(Player player) {
		if (player == player1) {
			return player2;
		} else {
			return player1;
	...
	OpenPlayerInfo otherThan(OpenPlayerInfo player) {
	...
	private int playerAction(Player player, OpenPlayerInfo playerInfo) {
	...
	private SimplePair<Player, OpenPlayerInfo> getPlayer(boolean onButton) {
	...
	private HandSummary roundCycle() {
		...
		while (result != ActionResult.SHOWDOWN && result != ActionResult.UNCONTESTED) {
			if (result == ActionResult.START_ROUND) {
				SimplePair<Player, OpenPlayerInfo> actingPlayer = getPlayer(true);
				...
				if (getBankRollAtRisk() > 0) {
					SimplePair<Player, OpenPlayerInfo> actingPlayer = getPlayer(false);
					result = playerAction(actingPlayer.first, actingPlayer.second);
				} else {
