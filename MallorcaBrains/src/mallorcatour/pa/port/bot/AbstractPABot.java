...
import mallorcatour.grandtorino.villainstat.PAVillainObserver;
...
public abstract class AbstractPABot implements Player {
    ...
    private BaseVillainModeller modeller;

    public AbstractPABot(String name) {
        DEBUG_PATH = name + "_debug_" + DateUtils.getDate(false) + ".txt";
        modeller = new BaseVillainModeller(LimitType.FIXED_LIMIT, DEBUG_PATH);
    ...
    protected BaseVillainModeller getVillainModeller() {
        return modeller;
    }
    ...
    public Action getAction() {
        Log.f(DEBUG_PATH, "Count of raises: " + paGameInfo.getNumRaises());
        mallorcatour.game.core.Action result = getRealBot().getAction();
